...
export default {
  ...
  setup(props){
    ...
    const submit= ()=>{
      ...
      .post(route('ingredient.update',props.id), {
        onSuccess: () => {
    ...
    const getIngredient=async ()=> {
      try {
        const response = await axios.get('/api/ingredient/show/'+props.id);
        const data = response.data;
        if(data.status=='success'){
          ingredientForm.value=useForm(data.data);

          console.log('ingredient',ingredientForm.value)
        }
      } catch (error) {
...
<template>
  ...
  <Layout>
    ...
    <PageHeader title="Edit Ingredient" pageTitle="Ingredient" />
    <BRow v-if="!isLoading">
      <BCol lg="12">
        <BCard no-body>
            <BCardHeader>
              <h5 class="card-title mb-0">Edit Ingredient</h5>
            </BCardHeader>
            <BCardBody>
              <BRow class="g-4">
                <!-- Nama -->
