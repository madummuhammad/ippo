        ...
        const destroy=(event)=>{
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const deleteEndpoint = 'api/ingredient_category/destroy/'+event.id;
...
<template>
  <Layout>
    ...
    <Head title="Ingredient Category" />
    <PageHeader title="Ingredient Category" pageTitle="Ingredient" />
    <BRow>
      <BCol xl="12" lg="12">
        <div>
          <BCard no-body>
            <BCardHeader class="border-0">
              <BRow class="g-4">
                <BCol sm="auto">
                  <div>
                    <Link :href="route('ingredient_category.create')" class="btn btn-success"><i class="ri-add-line align-bottom me-1"></i> Add
                    Ingredient Category</Link>
                  </div>
                </BCol>
                <BCol sm>
                  <div class="d-flex justify-content-sm-end">
                    <div class="search-box ms-2">
                      <input type="text" class="form-control" placeholder="Search Ingredient Category..." />
                  ...
                  <div id="table-product-list-all" class="table-card gridjs-border-none table-responsive">
                    <table class="table align-middle table-nowrap mb-0 mt-5" id="customerTable">
                      <thead class="table-light">
                        <tr class="text-muted">
                          <th class="sort" data-sort="product">CATEGORY NAME</th>
                          <th class="sort" data-sort="stock">DESCRIPTION</th>
                          <th scope="col">ACTION</th>
                        </tr>
                      </thead>
                      <tbody class="list form-check-all">
                        <tr class="gridjs-tr" v-for="(item,index) in categoryData">
                          <td data-column-id="product" class="gridjs-td">
                            <span>
                              <div class="d-flex align-items-center">
                                <div class="flex-grow-1">
                                  <h5 class="fs-14 mb-1">
                                    <Link :href="route('ingredient_category.edit',item.id)" class="text-body">{{item.name}}
                                    </Link>
                                  </h5>
                                </div>
                              </div>
                            ...
                            <span>
                              <div class="dropdown">
                                <BDropdown variant="link" toggle-class="btn btn-soft-secondary btn-sm dropdown arrow-none" menu-class="dropdown-menu-end" :offset="{ alignmentAxis: -130, crossAxis: 0, mainAxis: 10 }">
                                  ...
                                  <Link class="dropdown-item" :href="route('ingredient_category.edit',item.id)"> <i class="ri-pencil-fill align-bottom me-2 text-muted"></i>
                                  Edit
                                  </Link>
                    ...
                    <BCol xxl="12" sm="12" class="project-card text-center" v-if="categoryData.length==0">
                      <div class="p-5">
