...
export default {
  ...
  setup(props) {
    ...
    const getOutlet=async ()=> {
      try {
        const response = await axios.get('/api/outlet/show/'+props.id);
        const data = response.data;

        console.log('outlet',data);
        outletForm.value=useForm(data.data);
      } catch (error) {
        console.error('error', error);
      }
    }
    ...
    const submit = () => {
      ...
      outletForm.value.transform(data => ({
          ...data,
      })).post(route('outlet.update',props.id), {
          onSuccess: (response) => {
...
<template>
  <Layout>
    ...
    <PageHeader title="Edit Outlet" pageTitle="Outlet" />
    <BRow>
      <BCol lg="8">
        <BCard no-body>
          <BCardBody>
