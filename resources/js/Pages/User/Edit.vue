...
export default {
  ...
  setup(props){
    ...
    const submit= ()=>{
      ...
      .post(route('user.update',props.id), {
        onSuccess: () => {
    ...
    const getUser=async ()=> {
      try {
        const response = await axios.get('/api/user/show/'+props.id);
        const data = response.data;

        userForm.value.name=data.data.name;
        userForm.value.email=data.data.email;
        
        if (data.data.phone.startsWith('62')) {
            userForm.value.phone = data.data.phone.slice(2);
        } else {
            userForm.value.phone = data.data.phone;
        }

        userForm.value.outlet_id=data.data.outlet_id;
        userForm.value.role=data.data.role;
...
<template>
  ...
  <Layout>
    ...
    <PageHeader title="Edit User" pageTitle="Users" />
    <BRow>
      <BCol lg="12">
        <BCard no-body>
            <BCardHeader>
